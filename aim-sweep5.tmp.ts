// Confirm stability at the SHIPPED lr (0.05) for per-shot credit, and check the
// early trajectory (first 60 duels) — the shared brain only sees a handful of
// duels per tier in practice, so the aim should visibly center within ~20-40.
const F = 13;
...
function run(cfg: { aimSig: number; gscale: number; lr: number }) {
  let w = seedWeights();
  console.log(`\ncfg aimSig=${cfg.aimSig} gscale=${cfg.gscale} lr=${cfg.lr}  seed aimXb=${w.mov[2*F+12].toFixed(2)}`);
  let cumH = 0, cumS = 0;
  for (let n = 0; n < 300; n++) {
    ...
    if (n < 20 || n === 29 || n === 39 || n === 59 || (n + 1) % 50 === 0) {
      console.log(`  duel ${String(n + 1).padStart(3)}: aimXb=${w.mov[2*F+12].toFixed(3)} aimYb=${w.mov[3*F+12].toFixed(3)} | thisDuel hits=${ev.filter(e=>e.r===1).length}/${ev.length} | cum=${cumH}/${cumS} (${(100*cumH/cumS).toFixed(0)}%)`);
    }
  }
  const ax = w.mov[2 * F + 12];
  console.log(`  final: aimXb=${ax.toFixed(3)} (mu=${Math.tanh(ax).toFixed(2)})`);
}
run({ aimSig: 0.3, gscale: 0.3, lr: 0.05 });
run({ aimSig: 0.3, gscale: 0.2, lr: 0.05 });
run({ aimSig: 0.35, gscale: 0.25, lr: 0.05 });
