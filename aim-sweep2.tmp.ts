// Sweep #2: fix the baseline handling. Rows with G==0 (nothing happened near
// them) carry no information — skip them entirely instead of letting them
// inject noise through the mean baseline. Baseline computed over active rows.
const F = 13;
const MOVE_SIG = 0.85;
...
function grad(w: { mov: number[]; buttons: number[] }, rows: { f: number[]; s: number[]; x: number[]; y: number[]; fire: number[]; allow: number[]; feat: number[][]; t: number[] }, ev: { t: number; r: number }[], aimSig: number, gscale: number, mode: 'skip-zero' | 'full') {
  ...
  const active: number[] = [];
  let sum = 0;
  for (let i = 0; i < n; i++) { let g = 0; for (const e of ev) { const dt = e.t - rows.t[i]; if (dt <= 0 || dt > HORIZON) continue; g += e.r * Math.pow(GAMMA, dt); } G[i] = g; if (g !== 0) active.push(i); sum += g; }
  let bl = 0;
  if (mode === 'skip-zero') { bl = active.length ? sum / active.length : 0; } else { bl = sum / n; }
  for (let i = 0; i < n; i++) {
    if (mode === 'skip-zero' && !active.includes(i)) continue;
    const adv = G[i] - bl;
    if (adv === 0) continue;
...
function run(cfg: { aimSig: number; gscale: number; lr: number; mode: 'skip-zero' | 'full' }) {
  let w = seedWeights();
  ...
  for (let n = 0; n < 300; n++) {
    ...
    const { mg, bg } = grad(w, rows, ev, cfg.aimSig, cfg.gscale, cfg.mode);
    w = apply(w, mg, bg, cfg.lr);
  }
  const ax = w.mov[2 * F + 12], ay = w.mov[3 * F + 12];
  const pF = SIG(w.buttons[F + 12]);
  console.log(`mode=${cfg.mode} aimSig=${cfg.aimSig} gscale=${cfg.gscale} lr=${cfg.lr} → aimXw=${ax.toFixed(2)} aimYw=${ay.toFixed(2)} pF=${pF.toFixed(2)} hits all=${hit}/${shots} last50=${lastHits}/${lastShots}`);
}
run({ aimSig: 0.5, gscale: 0.3, lr: 0.02, mode: 'skip-zero' });
run({ aimSig: 0.6, gscale: 0.4, lr: 0.01, mode: 'skip-zero' });
run({ aimSig: 0.5, gscale: 0.5, lr: 0.008, mode: 'skip-zero' });
run({ aimSig: 0.4, gscale: 0.5, lr: 0.02, mode: 'skip-zero' });
run({ aimSig: 0.35, gscale: 0.5, lr: 0.03, mode: 'skip-zero' });
