...
// in db.ts; this router only validates query params and shapes the JSON.
// `sort` is whitelisted to kills|wins|accuracy (default kills) and `limit` is
// parsed/clamped to [1,100] (default 25), so no caller input ever reaches SQL
// unchecked. The main process mounts this under /api, exposing GET /api/leaderboard.
...
leaderboardRouter.get('/leaderboard', async (req, res) => {
  ...
  const leaderboard = await getLeaderboard({ sort, limit, window });
  ...
  const you = id ? await getPlayerRank(id, sort, window) : null;
