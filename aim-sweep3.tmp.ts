// Sweep #3: aim-axis gradient ONLY on rows where the bot actually fired (a shot
// event's cause is the aim sampled on that exact row; other rows' aim never
// produced a shot, so crediting them with shot outcomes is pure noise).
// Movement axes keep the full 12s horizon (positioning legitimately shapes the
// fight over seconds).
const F = 13;
...
function grad(w: { mov: number[]; buttons: number[] }, rows: { f: number[]; s: number[]; x: number[]; y: number[]; fire: number[]; allow: number[]; feat: number[][]; t: number[] }, ev: { t: number; r: number }[], aimSig: number, gscale: number, aimFireOnly: boolean) {
  ...
  let sum = 0, act = 0;
  for (let i = 0; i < n; i++) { let g = 0; for (const e of ev) { const dt = e.t - rows.t[i]; if (dt <= 0 || dt > HORIZON) continue; g += e.r * Math.pow(GAMMA, dt); } G[i] = g; if (g !== 0) { sum += g; act++; } }
  ...
  for (let i = 0; i < n; i++) {
    ...
    // Movement axes (f/s): full credit. Aim axes: only rows that FIRED —
    // their aim sample is the one that produced the shot events.
    const axes = [ [rows.f[i], o.muF, 0, MOVE_SIG, 1, true], [rows.s[i], o.muS, F, MOVE_SIG, 1, true] ];
    if (!aimFireOnly || rows.fire[i] === 1) {
      axes.push([rows.x[i], o.muX, 2 * F, aimSig, gscale, true], [rows.y[i], o.muY, 3 * F, aimSig, gscale, true]);
    }
    for (const [a, mu, off, sg, gs] of axes) { const deriv = ((a - mu) / (sg * sg)) * gs; const dMu = 1 - mu * mu; for (let f = 0; f < F; f++) mg[off + f] += deriv * dMu * rows.feat[i][f] * adv; }
...
function run(cfg: { aimSig: number; gscale: number; lr: number; aimFireOnly: boolean }) {
  let w = seedWeights();
  ...
  for (let n = 0; n < 300; n++) {
    ...
    const { mg, bg } = grad(w, rows, ev, cfg.aimSig, cfg.gscale, cfg.aimFireOnly);
    w = apply(w, mg, bg, cfg.lr);
  }
  const ax = w.mov[2 * F + 12], ay = w.mov[3 * F + 12];
  const pF = SIG(w.buttons[F + 12]);
  console.log(`fireOnly=${cfg.aimFireOnly} aimSig=${cfg.aimSig} gscale=${cfg.gscale} lr=${cfg.lr} → aimXw=${ax.toFixed(2)} aimYw=${ay.toFixed(2)} pF=${pF.toFixed(2)} hits all=${hit}/${shots} last50=${lastHits}/${lastShots}`);
}
run({ aimSig: 0.2, gscale: 0.235, lr: 0.05, aimFireOnly: true });
run({ aimSig: 0.35, gscale: 0.4, lr: 0.02, aimFireOnly: true });
run({ aimSig: 0.4, gscale: 0.5, lr: 0.02, aimFireOnly: true });
run({ aimSig: 0.5, gscale: 0.6, lr: 0.01, aimFireOnly: true });
run({ aimSig: 0.5, gscale: 0.5, lr: 0.008, aimFireOnly: false });
