// Sweep #4: PER-SHOT credit for aim + fire. Each shot event updates ONLY the
// aim/fire decisions on the row that fired it (no 12s smear, no episode
// baseline noise from unrelated rows). Movement axes + jump/dash/boost keep the
// episodic REINFORCE credit over frags/deaths only.
const F = 13;
...
// Per-shot gradient: for each fired row matched to its shot event, update the
// aim axes with raw reward (score function) and the fire head with reward minus
// the episode's mean shot reward (a per-shot baseline). Movement + jump/dash/
// boost use episodic credit over the big frag/death events (|r| >= 1).
function gradPerShot(w: { mov: number[]; buttons: number[] }, rows: { f: number[]; s: number[]; x: number[]; y: number[]; jump: number[]; fire: number[]; dash: number[]; boost: number[]; allowFire: number[]; feat: number[][]; t: number[] }, ev: { t: number; r: number }[], aimSig: number, gscale: number) {
  ...
  // --- episodic credit for movement (f/s) + jump/dash/boost, over |r|>=1 ---
  const bigEv = ev.filter((e) => Math.abs(e.r) >= 1);
  if (bigEv.length) {
    const G = new Array<number>(n).fill(0);
    let sum = 0, act = 0;
    for (let i = 0; i < n; i++) { let g = 0; for (const e of bigEv) { const dt = e.t - rows.t[i]; if (dt <= 0 || dt > HORIZON) continue; g += e.r * Math.pow(GAMMA, dt); } G[i] = g; if (g !== 0) { sum += g; act++; } }
    const bl = act ? sum / act : 0;
    for (let i = 0; i < n; i++) {
      if (G[i] === 0) continue;
      const adv = G[i] - bl;
      if (adv === 0) continue;
      const o = outputs(w, rows.feat[i]);
      for (const [a, mu, off, sg] of [[rows.f[i], o.muF, 0, MOVE_SIG], [rows.s[i], o.muS, F, MOVE_SIG]] as const) {
        const deriv = (a - mu) / (sg * sg); const dMu = 1 - mu * mu;
        for (let f = 0; f < F; f++) mg[off + f] += deriv * dMu * rows.feat[i][f] * adv;
      }
      const yv = [rows.jump[i], 0, rows.dash[i], rows.boost[i]];
      const pv = [o.pJ, 0, o.pD, o.pB];
      for (let b = 0; b < 4; b++) { if (b === 1) continue; const d = yv[b] - pv[b]; if (d === 0) continue; for (let f = 0; f < F; f++) bg[b * F + f] += d * rows.feat[i][f] * adv; }
    }
  }
  // --- per-shot credit for aim + fire ---
  const shotRewards: number[] = [];
  for (let i = 0; i < n; i++) if (rows.fire[i] === 1) {
    // match the event at (about) this row's time
    const evIdx = ev.findIndex((e) => Math.abs(e.t - rows.t[i]) < 1e-6);
    const r = evIdx >= 0 ? ev[evIdx].r : 0;
    shotRewards.push(r);
  }
  ...
  for (let i = 0; i < n; i++) {
    ...
    // fire head: Bernoulli update with per-shot baseline
    const advF = r - meanR;
...
function run(cfg: { aimSig: number; gscale: number; lr: number }) {
  ...
  for (let n = 0; n < 300; n++) {
    ...
    if (n >= 250) { for (const e of ev) { s50++; if (e.r === 1) h50++; } }
  ...
  console.log(`per-shot aimSig=${cfg.aimSig} gscale=${cfg.gscale} lr=${cfg.lr} → aimXw=${ax.toFixed(2)} aimYw=${ay.toFixed(2)} pF=${pF.toFixed(2)} hits all=${hit}/${shots} last50=${h50}/${s50}`);
}
run({ aimSig: 0.2, gscale: 0.235, lr: 0.05 });
run({ aimSig: 0.3, gscale: 0.3, lr: 0.03 });
run({ aimSig: 0.35, gscale: 0.35, lr: 0.02 });
run({ aimSig: 0.4, gscale: 0.4, lr: 0.02 });
run({ aimSig: 0.5, gscale: 0.5, lr: 0.01 });
